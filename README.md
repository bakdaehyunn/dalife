...
DaLife는 내가 남긴 기록과 생활 패턴을 기억하고, 그 근거를 바탕으로 맛집, 생활 루틴, 개인 코스를 추천하는 로컬 개인 생활 에이전트입니다.
DaLife 아카이브 엔진과 Momukbot의 맛집 추천 도메인, Honsanam Reminder의 생활 알림 도메인을 하나의 SQLite 기반 개인 컨텍스트 플랫폼으로 통합합니다.

제품, 저장소, Python 패키지, CLI, 환경 변수, 자동화 이름을 모두 **DaLife**로 통일합니다. 기본 명령은 `dalife`, 환경 변수 접두사는 `DALIFE_*`입니다.
...
DaLife은 흘러가는 캡처와 글을 아이디어 주머니에 담아두는 개인용 아카이브 시스템입니다. 공유와 전달은 Telegram 채팅방에서 하고, 저장은 로컬 폴더와 SQLite, 정리는 Codex 기반 processor가 맡습니다.
...
장기적으로 DaLife이 향하는 최종 제품 레이어는 Viewpoint Layer입니다. 저장된 캡처를 단순히 다시 찾는 데서 끝내지 않고, 내가 반복해서 보는 주제, 계속 남는 질문, 서로 이어지는 캡처, 프로젝트나 글감으로 발전할 수 있는 생각을 Codex와 다시 논의할 수 있는 재료로 만드는 방향입니다.

## 아카이브가 쌓이는 과정

1. Telegram에서 DaLife과 1:1 채팅을 엽니다.
2. 나중에 다시 보고 싶은 캡처나 사진을 보냅니다.
3. DaLife이 원본 메시지와 파일을 로컬에 저장합니다.
...
이 구조 덕분에 DaLife은 조용히 돌아가면서도 원본 보관, 관심사 분류, 요약, 나중에 다시 볼 이유, 관심사 그래프, 사용자의 선택 기록을 분리해서 남길 수 있습니다.
...
DaLife은 이제 세 개의 독립 도구를 한 번에 지우는 방식이 아니라, 동작을 보존하면서 하나의 canonical repo로 옮기는 방식으로 통합됩니다.

- `archive`: 기존 DaLife 캡처, 처리, 검색, 리뷰, graph/export 흐름입니다.
...
SQLite가 운영 데이터의 source of truth이고, RDF graph와 JSON-LD export는 검증된 SQLite row에서 다시 만들 수 있는 파생물입니다. 기존 `momuk`과 `honsanam-reminder` CLI는 제거되었으며 모든 기능은 `dalife`의 도메인 명령으로 실행합니다.

## 로컬 아카이브에 정리되는 내용

DaLife은 Telegram으로 보낸 캡처와 메시지를 로컬 폴더와 SQLite DB에 저장하고, 나중에 찾아보기 쉬운 형태로 정리합니다.
...
```bash
cd /Users/hennei/workspace/dalife
...
DALIFE_ALLOW_ALL_CHATS=false
```

테스트된 설정 흐름:

```bash
dalife setup --non-interactive \
  --telegram-bot-token '<BOT_TOKEN>' \
  --telegram-chat-id '<CHAT_ID>' \
  --telegram-admin-user-id '<YOUR_USER_ID>'
dalife doctor --online
dalife telegram-commands sync
dalife send-test --allowed
```

같은 setup 명령에서 `--kakao-rest-api-key`, `--naver-client-id`,
`--naver-client-secret`도 설정할 수 있습니다.
...
```bash
dalife telegram
dalife telegram-digest --kind revisit --dry-run
```
...
dalife doctor
dalife doctor --online
dalife discover-chat
dalife rooms
dalife telegram-commands show
dalife telegram-commands sync
dalife telegram
dalife pending
dalife process
dalife process --export-graph
dalife telegram-digest
dalife telegram-digest --kind project-seed
dalife telegram-digest --kind weekly
...
dalife archive list
dalife archive search <query>
dalife archive review
dalife archive show <capture-id>
dalife food parse "오목교역 곱창 맛집 추천"
dalife food plan-collection --area 신정동 --daily-quota-limit 30
dalife food due-queries
dalife food import-provider-config --source-env /path/to/momukbot/.env --dry-run
dalife food import-momuk-history --root /path/to/momukbot --dry-run
dalife food plan-history-refresh --area 신정동 --legacy-area "현재 위치" --persist
dalife food quota --json
dalife food run-collection --max-queries 20 --max-quota-cost 20 --dry-run
dalife food recommend-local --area 신정동 --topic "고기 저녁" --count 10
dalife life list
dalife life next --date 2026-08-29 --days 7
dalife life preview --date 2026-08-29 --time 09:00
dalife life import-honsanam --root /path/to/honsanam-reminder-bot --dry-run
dalife life run-once --dry-run --json
dalife life show trash --json
dalife life add custom --id water-plants --title "화분 물주기" --kind weekly --time 09:30 --weekday sun --action "화분에 물 주기"
dalife life pending --json
dalife life pattern show
dalife course plan --area 이태원 --date 2026-08-29 --time 18:30
dalife schedule plan
dalife schedule cutover-check
python -m dalife.launchd write . --include-life-sender
dalife graph init
dalife graph sync
...
dalife search <query>
dalife search <query> --rebuild
dalife review
dalife review --needs-review
dalife review --revisit
dalife web
dalife interests
dalife concepts
...
dalife process --no-codex
dalife list
dalife show <capture-id>
dalife send-test --chat-id <telegram-chat-id>
```
...
- `archive ...`: 기존 archive 관련 flat command의 grouped alias입니다. `dalife list`와 `dalife archive list`는 같은 로컬 archive 데이터를 봅니다.
...
- `food import-provider-config`: 기존 Momukbot `.env`에서 Kakao/Naver 설정만 가져오며 기존 DaLife 설정과 secret 출력은 건드리지 않습니다.
...
- `DALIFE_NATIVE_PERSONAL_TELEGRAM=true`: Telegram 맛집, 생활 알림 조회, 코스 요청을 archive capture 대신 SQLite 기반 native handler로 처리하고, 맛집 상위 후보의 좋아요/별로/방문/숨김/비슷한 곳 feedback 버튼을 활성화합니다. 기본값은 `false`이며 검증된 방에서 전환하기 전까지 기존 capture 동작을 유지합니다.
- `DALIFE_LIFE_TIMEZONE=Asia/Seoul`: 생활 알림의 현재 시각과 CLI 입력 시각을 해석할 timezone입니다. 기존 동작과의 호환을 위해 기본값은 `Asia/Seoul`입니다.
...
- `graph export`: 정리된 아카이브 항목을 `.local/graph/dalife.jsonld`에 lightweight JSON-LD export로 내보냅니다. 전체 semantic store 백업이 아니라 사람이 확인하기 쉬운 휴대용 export입니다. 기본값은 원문 전체를 내보내지 않습니다.
...
DaLife은 단순히 "이미지를 저장했다"에서 끝나지 않고, 나중에 검색하고 다시 읽기 좋은 형태로 정리합니다.
...
정리 품질이 약한 항목은 `dalife reprocess-plan`으로 먼저 확인합니다. 이 단계는 관련 캡처, 반복 주제, insight note로 넘어가기 전에 아카이브의 관심사, 주제, 핵심 포인트, insight seed가 충분히 채워졌는지 확인하는 안전장치입니다. 실제 재처리는 `dalife reprocess --capture-id <capture-id>`처럼 하나의 캡처를 명시할 때만 실행합니다.

Draft insight note는 `dalife insights generate --period weekly`로 로컬에 생성할 수 있습니다. 평소에는 `dalife telegram-digest --kind weekly`가 먼저 짧은 Telegram 확인 카드로 "이번 주 insight draft를 만들지" 물어보는 흐름을 담당합니다. 실제 insight note 본문과 근거 archive item 확인은 SQLite와 로컬 명령에서 이뤄집니다.

## 주요 설정

```env
DALIFE_CODEX_ENABLED=true
DALIFE_CODEX_BIN=codex
DALIFE_CODEX_MODEL=
DALIFE_CODEX_SANDBOX=read-only
DALIFE_CODEX_EPHEMERAL=true
DALIFE_CODEX_TIMEOUT_SEC=900
```

`DALIFE_CODEX_BIN=codex`처럼 명령 이름만 넣어도 DaLife은 일반 PATH와 Homebrew 기본 경로(`/opt/homebrew/bin`, `/usr/local/bin`)에서 실행 파일을 찾습니다. launchd 환경이 터미널보다 PATH가 짧을 수 있으므로, 문제가 있으면 `/opt/homebrew/bin/codex`처럼 절대 경로로 적어도 됩니다.

## 앞으로 더해질 수 있는 것

DaLife은 먼저 캡처와 글을 안정적으로 모으고 관심사별로 정리하는 개인 아카이브로 시작합니다. 이후에는 Viewpoint Layer로 확장해 저장된 항목 사이의 관련성, 반복되는 관심사, 주간/월간 인사이트, Codex 논의 맥락, Telegram으로 다시 꺼내보기 같은 기능을 붙일 수 있습니다.
...
- `com.hennei.dalife.telegram`: Telegram polling bot을 계속 실행합니다.
- `com.hennei.dalife.processor`: 5분마다 `dalife process --export-graph`를 실행합니다.
- `com.hennei.dalife.digest`: 매일 오전 9시에 `dalife telegram-digest --kind revisit`를 실행해 재방문 후보를 추천합니다.
- `com.hennei.dalife.digest.project-seed`: 매일 오후 6시에 `dalife telegram-digest --kind project-seed`를 실행해 프로젝트 씨앗 후보를 추천합니다.
- `com.hennei.dalife.digest.weekly`: 매일 오전 9시 30분에 `dalife telegram-digest --kind weekly`를 실행합니다. 주간 prompt key가 같으면 이미 보낸 prompt는 다시 보내지 않습니다.

일반 운영에서는 launchd가 poller를 계속 켜고 processor를 5분마다 실행하며, digest jobs가 오래된 아카이브에서 다시 볼 만한 후보, 프로젝트 씨앗 후보, 주간 insight 후보를 추천합니다. Processor는 먼저 SQLite에서 pending capture와 backoff 시간이 지난 retry 대상만 확인하고, 없으면 Codex를 호출하지 않고 종료합니다. 처리된 항목이 있을 때만 그래프 export를 실행합니다. 반복 실패한 항목은 `failed_blocked`로 멈춰 같은 실패가 5분마다 계속 쌓이지 않게 합니다. `dalife telegram`, `dalife pending`, `dalife process`, `dalife telegram-digest --dry-run` 직접 실행은 테스트/디버그용입니다.

## Troubleshooting

- `dalife list`에 아무 것도 없으면 `dalife telegram`이 실행 중인지 확인합니다.
