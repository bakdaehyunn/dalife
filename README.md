...
Darchive 아카이브 엔진과 Momukbot의 맛집 추천 도메인, Honsanam Reminder의 생활 알림 도메인을 하나의 SQLite 기반 개인 컨텍스트 플랫폼으로 통합합니다.

제품과 Telegram에서 보이는 이름은 **DaLife**이며, 기존 자동화와 CLI 호환성을 위해 저장소, Python 패키지, `darchive` 명령, `DARCHIVE_*` 환경 변수에는 Darchive 기술 식별자를 유지합니다.
