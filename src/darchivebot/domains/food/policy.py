...
def is_excluded_general_text(text: str, *, allow_fast_food: bool = False, case_sensitive: bool = True) -> bool:
    excluded_name_words = GENERAL_EXCLUDED_NAME_WORDS
    excluded_category_words = GENERAL_EXCLUDED_CATEGORY_WORDS
    if allow_fast_food:
        excluded_name_words = tuple(word for word in excluded_name_words if word not in FAST_FOOD_EXCLUDED_NAME_WORDS)
