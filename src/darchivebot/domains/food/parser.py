...
from darchivebot.domains.food.models import FoodParsedRequest
...
def parse_food_request(text: str, *, default_count: int = 30) -> FoodParsedRequest:
    raw = text.strip()
    if not looks_like_food_request(raw):
        return FoodParsedRequest(intent="unknown", count=default_count)
