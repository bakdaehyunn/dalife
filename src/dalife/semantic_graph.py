...
from dalife.archive_values import json_string_list, semantic_string_list
from dalife.models import ArchiveItemRecord
from dalife.ontology import (
...
from dalife.ports import ArchiveRepositoryPort
from dalife.time_utils import utc_now


DARCH = "https://dalife.local/ontology#"
...
SEMANTIC_GRAPH_NAME = NamedNode("https://dalife.local/graph/semantic")
...
def sync_semantic_store(
    archive_store: ArchiveRepositoryPort,
    path: Path,
    *,
...
def semantic_store_stats_from_store(store: Store, path: Path) -> dict[str, Any]:
    stats = {
        ...
        "raw_text_included": bool_query(store, "ASK { GRAPH <https://dalife.local/graph/semantic> { ?s <https://dalife.local/ontology#rawExtractedText> ?o } }"),
    }
    sync_rows = list(
        store.query(
            """
            SELECT ?generated_at ?ontology_version ?export_version WHERE {
              GRAPH <https://dalife.local/graph/semantic> {
                <urn:dalife:graph-sync:current> <https://dalife.local/ontology#generatedAt> ?generated_at ;
                  <https://dalife.local/ontology#ontologyVersion> ?ontology_version ;
                  <https://dalife.local/ontology#exportVersion> ?export_version .
              }
            }
            LIMIT 1
            """
...
def quads_for_archive_rows(rows: list[ArchiveItemRecord], *, include_raw_text: bool = False) -> Iterable[Quad]:
    for row in rows:
        yield from quads_for_archive_row(row, include_raw_text=include_raw_text)


def quads_for_archive_row(row: ArchiveItemRecord, *, include_raw_text: bool = False) -> Iterable[Quad]:
    ...
    for question in questions:
        question_node = node(question_id(question))
    ...
    for candidate in relation_candidates:
        relation_node = node(relation_candidate_id(archive_id, candidate))
...
def metadata_quads(
    *,
    ...
    sync_node = node("urn:dalife:graph-sync:current")
    values = {
...
def count_type(store: Store, class_name: str) -> int:
    return count_query(
        ...
        SELECT (COUNT(?s) AS ?count) WHERE {{
          GRAPH <https://dalife.local/graph/semantic> {{
            ?s a <https://dalife.local/ontology#{class_name}> .
          }}
        }}
        """,
    )
