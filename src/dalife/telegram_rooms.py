...
from dalife.telegram_messages import chat_display_name, object_value
...
class TelegramRoomState:
    dalife_chat_id: str = ""
    dalife_chat_title: str = ""
    dalife_chat_type: str = ""
...
def read_room_state(settings: Settings) -> TelegramRoomState:
    ...
    return TelegramRoomState(
        dalife_chat_id=str(payload.get("dalife_chat_id") or "").strip(),
        dalife_chat_title=str(payload.get("dalife_chat_title") or "").strip(),
        dalife_chat_type=str(payload.get("dalife_chat_type") or "").strip(),
...
def read_registered_chat_id(settings: Settings) -> str:
    return read_room_state(settings).dalife_chat_id
...
def format_rooms_report(settings: Settings) -> tuple[int, str]:
    ...
    if state.dalife_chat_id:
        lines.extend(
            [
                f"dalife_chat_id={mask_identifier(state.dalife_chat_id)}",
                f"title={state.dalife_chat_title or '(empty)'}",
                f"type={state.dalife_chat_type or '(empty)'}",
                ...
                f"allowed={'yes' if state.dalife_chat_id in allowed_chat_ids(settings) else 'no'}",
            ]
        )
    else:
        lines.append(f"[WARN] dalife_chat_id is not registered; send {REGISTER_CHAT_ROOM_COMMAND} in the Telegram chat")
