...
from dalife.config import Settings, ensure_local_dirs
from dalife.ports import DatabasePort
...
def run_doctor(
    ...
    *,
    ...
    if settings.telegram_bot_token:
        ...
        lines.append("[TODO] Add TELEGRAM_BOT_TOKEN to .env, then run: dalife doctor --online")
    ...
    if settings.telegram_allowed_chat_ids:
        ...
        lines.append("[WARN] DALIFE_ALLOW_ALL_CHATS=true; every chat can use the bot")
    else:
    ...
    lines.append("[INFO] Normal operation: launchd/cron keeps capture running and runs `dalife process --export-graph` every 5 minutes")
    ...
    lines.append("[INFO] Test/debug commands: run `dalife telegram`, `dalife pending`, or `dalife process` directly")
    ...
    if online:
        ...
        lines.append("[WARN] Telegram API checks skipped; run: dalife doctor --online")
...
def describe_room_state(settings: Settings) -> tuple[int, list[str]]:
    ...
    if state.dalife_chat_id:
        lines = [f"[OK] dalife_chat_id is registered: {mask_identifier(state.dalife_chat_id)}"]
        if state.dalife_chat_id in settings.telegram_allowed_chat_ids:
            lines.append("[OK] dalife_chat_id is also listed in TELEGRAM_ALLOWED_CHAT_IDS")
        else:
            lines.append("[OK] dalife_chat_id is allowed by runtime registration")
        return 0, lines
    if settings.telegram_allowed_chat_ids:
        return 0, ["[WARN] dalife_chat_id is not registered; env allow-list can still capture messages"]
    return 0, ["[WARN] dalife_chat_id is not registered; send /set_chat_room in the Telegram chat"]
...
def describe_telegram_api_state(
    ...
    try:
        ...
        if command_menu_is_synced(commands, DEFAULT_BOT_COMMANDS):
            ...
            lines.append("[TODO] Run: dalife telegram-commands sync")
    except Exception as exc:
    ...
    if state.dalife_chat_id:
        try:
            commands = api.get_my_commands(scope=chat_command_scope(state.dalife_chat_id))
...
def describe_recent_polling_conflicts(settings: Settings) -> list[str]:
    ...
    return [
        ...
        "[TODO] Make sure only one `dalife telegram` process uses this bot token, and do not share polling tokens across bots",
    ]
