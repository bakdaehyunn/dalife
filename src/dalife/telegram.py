...
from dalife.domains.life import apply_life_callback, parse_life_callback_data
...
from dalife.telegram_course import plan_course_for_telegram
from dalife.telegram_life import upcoming_life_for_telegram
from dalife.telegram_messages import (
...
from dalife.telegram_prompts import (
    choice_label,
...
from dalife.telegram_rooms import (
...
class TelegramCaptureBot:
    ...
    def can_overwrite_registered_room(self, chat_id: str, chat: dict[str, Any], text: str) -> bool:
        ...
        if not state.dalife_chat_id or state.dalife_chat_id == chat_id:
            return True
        ...
        self.api.send_message(
            ...
            (
                ...
                f"기존: {state.dalife_chat_id} / {state.dalife_chat_title or '(empty)'}\n"
    ...
    def save_registered_room(self, chat_id: str, chat: dict[str, Any], message: dict[str, Any]) -> None:
        ...
        user = object_value(message.get("from"))
        data = {
            "dalife_chat_id": chat_id,
            "dalife_chat_title": chat_display_name(chat),
            "dalife_chat_type": str(chat.get("type") or ""),
...
def build_logger(settings: Settings) -> logging.Logger:
    ...
    logger = logging.getLogger("dalife.telegram")
    if logger.handlers:
        return logger
