...
from dalife.cli_formatting import mask_identifier
from dalife.config import DEFAULT_ENV_FILE, Settings, ensure_local_dirs, get_settings, update_env_values
from dalife.doctor import run_doctor
from dalife.storage import ArchiveStore
from dalife.telegram import TelegramApiClient, discover_chat_candidates
...
def setup_cmd_impl(
    ...
    *,
    ...
    print("Test/debug mode: run `dalife telegram`, `dalife pending`, or `dalife process` directly when checking behavior.")
...
def discover_chat_for_setup(token: str, *, dry_run: bool, non_interactive: bool) -> str:
    ...
    if dry_run:
        print("[dry-run] dalife discover-chat --plain")
        return ""
...
def write_setup_env(
    ...
    *,
    ...
    values = {
        ...
        "DALIFE_ALLOW_ALL_CHATS": str(allow_all_chats).lower(),
        "DALIFE_LIFE_TIMEZONE": life_timezone,
        "DALIFE_STATE_DIR": ".local/state",
        "DALIFE_LOG_DIR": ".local/logs",
        "DALIFE_MEDIA_DIR": ".local/captures",
        ...
        "DALIFE_PROCESSOR_BATCH_SIZE": "10",
        "DALIFE_TESSERACT_BIN": "tesseract",
    }
    values.update(
