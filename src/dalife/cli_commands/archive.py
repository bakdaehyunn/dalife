...
from dalife.archive_values import archive_item_to_dict, record_to_dict as row_to_dict
...
from dalife.readiness import concept_summary, interest_summary, related_captures, reprocess_plan
from dalife.search import rebuild_search_index, review_queue, search_archive
from dalife.semantic_graph import default_semantic_store_path, sync_semantic_store
...
def reprocess_dry_run_cmd(
    ...
    *,
    ...
    payload = {
        ...
        "message": "No SQLite rows were changed. Run `dalife reprocess --capture-id <capture-id>` to reprocess one selected capture.",
    }
    if json_output:
        print(json.dumps(payload, ensure_ascii=False, indent=2))
        return 0
