...
from dalife.insights import list_insight_notes, show_insight_note
from dalife.ports import WebStore
from dalife.search import archive_detail, rebuild_search_index, review_queue, search_archive
...
def serve_local_web(store: WebStore, *, host: str = "127.0.0.1", port: int = 8765) -> None:
    if host not in LOCAL_HOSTS:
        raise ValueError("DaLife web UI is local-only; use 127.0.0.1, localhost, or ::1")
    handler = make_handler(store)
    server = ThreadingHTTPServer((host, port), handler)
    print(f"dalife local web UI listening on http://{host}:{server.server_port}")
    server.serve_forever()


def make_handler(store: WebStore) -> type[BaseHTTPRequestHandler]:
    class DaLifeWebHandler(BaseHTTPRequestHandler):
