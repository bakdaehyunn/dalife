...
from darchivebot.config import get_settings, read_env_values, resolve_executable, update_env_values
...
def test_native_personal_telegram_is_off_by_default_and_explicitly_enabled(monkeypatch, tmp_path):
    monkeypatch.delenv("DARCHIVE_NATIVE_PERSONAL_TELEGRAM", raising=False)
    assert get_settings(tmp_path).native_personal_telegram_enabled is False

    monkeypatch.setenv("DARCHIVE_NATIVE_PERSONAL_TELEGRAM", "true")
...
def test_life_timezone_defaults_to_seoul_and_can_be_configured(monkeypatch, tmp_path):
    monkeypatch.delenv("DARCHIVE_LIFE_TIMEZONE", raising=False)
    assert get_settings(tmp_path).life_timezone == "Asia/Seoul"

    monkeypatch.setenv("DARCHIVE_LIFE_TIMEZONE", "America/New_York")
